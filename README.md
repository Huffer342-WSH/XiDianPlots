...
with plt.style.context(["xidian"]):
    fig, ax = plt.subplots()

    ax.plot(x, y1, "r-", label=r"Theory: $e^{-\alpha t} \sin(\omega t)$")
    ax.plot(x, y2, "b--", label=r"Experiment: $\cos(\theta)$")
    ax.set_xlabel(r"时间 Time ($t$)")
    ax.set_ylabel(r"幅值 Amplitude ($A$)")
    ax.set_title(r"系统响应 System Response (标准模式)")
    ax.legend(loc="upper right")

    # 默认后端
    fig.savefig("demo_standard.png", dpi=300)
    fig.savefig("demo_standard.pdf")

    # 使用pgf后端，调用xelatex生成图片
    fig.savefig("demo_latex.png", dpi=300, backend="pgf")
    fig.savefig("demo_latex.pdf", backend="pgf")

    # 简单封装
    XiDianPlots.savefig(fig,"demo_xd.png")
